...
class Seq2SeqTransformerWithTokensClassLabelingTrain(Seq2SeqTransformerWithTokensClassifierBase):
    ...
    name_for_saving = "transformer_with_tokens_class_labeling"  # TODO: Добавить _train в конец?
...
class Seq2SeqTransformerWithTokensClassLabelingEval(Seq2SeqTransformerWithTokensClassLabelingBase):
    ...
    name_for_saving = "transformer_with_tokens_class_labeling"  # TODO: Добавить _val в конец?
...
class Seq2SeqTransformerWithClassEmbeddingsGroupDecoderTrain(Seq2SeqTransformerWithEncoderForClasses):
    ...
    def _prepare_batch(self, batch):
        ...
        embeds_with_classes = torch.stack(embeds_with_classes, dim=1)  # S x B x 320
    ...
    def _prepare_batch(self, src: torch.Tensor, src_mask: torch.Tensor, src_padding_mask: torch.Tensor):
        ...
        for batch_i, (classes, tokens, pad_mask) in enumerate(zip(batch_classes, batch_tokens, batch_pad_mask)):
            ...
            for begin, end in zip(idx, idx[1:]):
                ...
                cur_memory = memory[:, batch_i].clone()  # S x H (H = 320)
        ...
        # None here stands for targets (see Train variant of model).
...
def _memory_greedy_decode(model, memory, memory_key_padding_mask, plain_groups, src_lengths, max_additional_tokens, start_symbol, tokenizer, device):
    ...
    for cur_len in range(1, max(batch_lengths)):
        cuda_result = result.to(device)
        tgt_mask, tgt_padding_mask = create_mask(tokenizer, tgt=cuda_result)

        out = model.decode(cuda_result, memory, tgt_mask,
                           tgt_padding_mask, memory_key_padding_mask)
        prob = model.generator(out[-1])
        next_word = torch.argmax(prob, dim=-1).cpu()
        next_word[eos_check | (cur_len >= batch_lengths)] = tokenizer.pad_token_id
    ...
    return result, plain_groups  # .T — for batch first
...
@torch.inference_mode()
def seq2seq_transformer_classes_groups_greedy_decode(model, src, max_additional_tokens,
...
@torch.inference_mode()
def seq2seq_transformer_classes_groups_translate(model, tokenizer, device, srcs, max_additional_tokens=5):
    model = model.to(device)
    model.eval()
    ...
    translations, plain_groups = seq2seq_transformer_classes_groups_greedy_decode(
        model, src, max_additional_tokens, tokenizer.bos_token_id, tokenizer, device
    )
...
@torch.inference_mode()
def model6_greedy_decode(model, batch, max_additional_tokens, start_symbol, tokenizer, device):
    model = model.to(device)
    model.eval()
...
@torch.inference_mode()
def model6_translate(model, tokenizer, device, srcs, max_additional_tokens=5):
    model = model.to(device)
    model.eval()

    translations, plain_groups = model6_greedy_decode(
        model, srcs, max_additional_tokens, tokenizer.bos_token_id, tokenizer, device
    )
