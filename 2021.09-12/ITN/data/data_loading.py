    ...
    for elem in batch:
        ...
        for begin, end in zip(idx, idx[1:]):
            ...
            if _check_token_is_plain(cls):
                new_tokens += all_spoken_ids[sp_prefix_len - len(spoken_ids):sp_prefix_len]
            else:
...
def get_datasets(tokenizer_name, language, data_target):  # =DEFAULT_TOKENIZER_NAME, ="ru"
    ...
    dataset_path = f"{data_paths[language]}/dataset_{{}}_{tokenizer_name}_tokenized"

    if data_target == "seq2seq":
        dataset_path += ""
    elif data_target == "classes":
        dataset_path += "_for_classes"

    return tuple(datasets.load_from_disk(dataset_path.format(mode)) for mode in ("train", "val"))
